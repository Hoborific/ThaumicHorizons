...
public class RenderEventHandler
{
                ...
                for (final String key : this.fociHover.keySet()) {
                    if (this.fociHover.get(key)) {
                        if (!THKeyHandler.radialActive && !THKeyHandler.radialLock) {
                            PacketHandler.INSTANCE.sendToServer((IMessage)new PacketLensChangeToServer((EntityPlayer)mc.thePlayer, key));
                            THKeyHandler.radialLock = true;
                        }
    ...
    private void renderFocusRadialHUD(final double sw, final double sh, final long time, final float partialTicks) {
        ...
        UtilsFX.renderQuadCenteredFromTexture(width * 2.75f * RenderEventHandler.radialHudScale, 0.5f, 0.5f, 0.5f, 200, 771, 0.5f);
        GL11.glDisable(3042);
        ...
        GL11.glEnable(3042);
        GL11.glBlendFunc(770, 771);
        UtilsFX.renderQuadCenteredFromTexture(width * 2.55f * RenderEventHandler.radialHudScale, 0.5f, 0.5f, 0.5f, 200, 771, 0.5f);
        GL11.glDisable(3042);
        GL11.glAlphaFunc(516, 0.1f);
        GL11.glPopMatrix();
        if (lens != null) {
        ...
        GL11.glScaled((double)RenderEventHandler.radialHudScale, (double)RenderEventHandler.radialHudScale, (double)RenderEventHandler.radialHudScale);
        ...
        GL11.glPopMatrix();
        if (tt != null) {
            UtilsFX.drawCustomTooltip(mc.currentScreen, ri, mc.fontRenderer, tt.getTooltip((EntityPlayer)mc.thePlayer, mc.gameSettings.advancedItemTooltips), -4, 20, 11);
        }
    ...
    void setBlocksEvanescent(final EntityPlayer p) {
        if (p.isSwingInProgress) {
            ...
            if (this.breakProgress > 1.0f) {
                Minecraft.getMinecraft().getNetHandler().addToSendQueue((Packet)new C07PacketPlayerDigging(2, p.getEntityId(), this.cacheX, this.cacheY, this.cacheZ));
        ...
        for (int i = -1; i < 2; ++i) {
            for (int j = -1; j < 2; ++j) {
                if (this.tempDir == ForgeDirection.UP || this.tempDir == ForgeDirection.DOWN) {
                    if (p.worldObj.getTileEntity(this.cacheX + i, this.cacheY, this.cacheZ + j) == null && p.worldObj.getBlock(this.cacheX + i, this.cacheY, this.cacheZ + j).getLightValue() == 0 && p.worldObj.getBlock(this.cacheX + i, this.cacheY, this.cacheZ + j).canCollideCheck(p.worldObj.getBlockMetadata(this.cacheX + i, this.cacheY, this.cacheZ + j), false)) {
                    ...
                    if (p.worldObj.getTileEntity(this.cacheX + i, this.cacheY + j, this.cacheZ) == null && p.worldObj.getBlock(this.cacheX + i, this.cacheY + j, this.cacheZ).getLightValue() == 0 && p.worldObj.getBlock(this.cacheX + i, this.cacheY + j, this.cacheZ).canCollideCheck(p.worldObj.getBlockMetadata(this.cacheX + i, this.cacheY + j, this.cacheZ), false)) {
                ...
                else if (p.worldObj.getTileEntity(this.cacheX, this.cacheY + j, this.cacheZ + i) == null && p.worldObj.getBlock(this.cacheX, this.cacheY + j, this.cacheZ + i).getLightValue() == 0 && p.worldObj.getBlock(this.cacheX, this.cacheY + j, this.cacheZ + i).canCollideCheck(p.worldObj.getBlockMetadata(this.cacheX, this.cacheY + j, this.cacheZ + i), false)) {
    ...
    public void resetBlocks(final EntityPlayer p) {
        ...
        for (int i = -1; i < 2; ++i) {
            for (int j = -1; j < 2; ++j) {
                if (this.tempDir == ForgeDirection.UP || this.tempDir == ForgeDirection.DOWN) {
                    if (this.tempBlock[i + 1][j + 1] != null && p.worldObj.getBlock(this.cacheX + i, this.cacheY, this.cacheZ + j) == ThaumicHorizons.blockEvanescent) {
                        p.worldObj.setBlock(this.cacheX + i, this.cacheY, this.cacheZ + j, this.tempBlock[i + 1][j + 1], this.tempMD[i + 1][j + 1], 4);
                    }
                }
                else if (this.tempDir == ForgeDirection.NORTH || this.tempDir == ForgeDirection.SOUTH) {
                    if (this.tempBlock[i + 1][j + 1] != null && p.worldObj.getBlock(this.cacheX + i, this.cacheY + j, this.cacheZ) == ThaumicHorizons.blockEvanescent) {
                        p.worldObj.setBlock(this.cacheX + i, this.cacheY + j, this.cacheZ, this.tempBlock[i + 1][j + 1], this.tempMD[i + 1][j + 1], 4);
                    }
                }
                else if (this.tempBlock[i + 1][j + 1] != null && p.worldObj.getBlock(this.cacheX, this.cacheY + j, this.cacheZ + i) == ThaumicHorizons.blockEvanescent) {
    ...
    public void renderLast(final RenderWorldLastEvent event) {
        for (final EntityLivingBase entity : this.thingsThatSparkle) {
            if (Minecraft.getMinecraft().thePlayer.getDistanceSqToEntity((Entity)entity) < 32.0 && entity.worldObj.rand.nextFloat() > 0.95f) {
                final float angle = (float)(entity.worldObj.rand.nextFloat() * 2.0f * 3.141592653589793);
                Thaumcraft.proxy.sparkle((float)entity.posX + entity.width * (float)Math.cos(angle), (float)entity.posY + entity.height * (entity.worldObj.rand.nextFloat() - 0.1f) * 1.2f, (float)entity.posZ + entity.width * (float)Math.sin(angle), 2.0f, 7, 0.0f);
            }
        }
    ...
    public void clearWater(final RenderBlockOverlayEvent event) {
        ...
        if (event.overlayType == RenderBlockOverlayEvent.OverlayType.FIRE && mc.thePlayer.ridingEntity != null && mc.thePlayer.ridingEntity instanceof EntityBoatThaumium) {
            event.setCanceled(true);
        }
    }
