...
public class TileRecombinatorRender extends TileEntitySpecialRenderer
{
    ...
    public void renderTileEntityAt(final TileEntity te, final double x, final double y, final double z, final float f) {
        ...
        if (tco.activated) {
            ...
            GL11.glPushMatrix();
            UtilsFX.renderFacingQuad(tco.xCoord + 0.5, tco.yCoord + 1.5 + 0.2 * sin, tco.zCoord + 0.5, 0.0f, 1.5f, 0.9f, frames, i, f, 16777215);
            GL11.glPopMatrix();
            if (te.getWorldObj().getTileEntity(te.xCoord, te.yCoord - 1, te.zCoord) instanceof TileNode) {
                Thaumcraft.proxy.beam((World)Minecraft.getMinecraft().theWorld, te.xCoord + 0.5, te.yCoord + 0.2, te.zCoord + 0.5, te.xCoord + 0.5, te.yCoord - 0.5, te.zCoord + 0.5, 1, 16777215, false, 0.1f, 3);
            }
            GL11.glDisable(3042);
        }
        GL11.glPopMatrix();
