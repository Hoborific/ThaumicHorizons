...
public class TileTransductionAmplifierRender extends TileEntitySpecialRenderer
{
    ...
    public void renderTileEntityAt(final TileTransductionAmplifier tile, final double par2, final double par4, final double par6, final float par8) {
        int bright = 20;
        GL11.glPushMatrix();
        ...
        if (tile.getWorldObj() != null) {
            final float scale = MathHelper.sin(Minecraft.getMinecraft().renderViewEntity.ticksExisted / 3.0f) * 0.1f + 0.9f;
        ...
        for (int a = 0; a < 4; ++a) {
            ...
            GL11.glColor4f(0.8f, 0.8f, 0.0f, 1.0f);
            if (tile.getWorldObj() != null) {
                final float scale2 = MathHelper.sin((Minecraft.getMinecraft().renderViewEntity.ticksExisted + a * 5) / 3.0f) * 0.1f + 0.9f;
