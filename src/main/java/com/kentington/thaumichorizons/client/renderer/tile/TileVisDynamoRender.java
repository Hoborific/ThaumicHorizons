...
public class TileVisDynamoRender extends TileEntitySpecialRenderer
{
    ...
    public void renderTileEntityAt(final TileEntity te, final double x, final double y, final double z, final float f) {
        ...
        if (tco.rise >= 0.3f && tco.ticksProvided > 0) {
            ...
            UtilsFX.renderFacingQuad(tco.xCoord + 0.5, (double)(tco.yCoord + 0.5f), tco.zCoord + 0.5, 0.0f, 0.2f, 0.9f, frames, i, f, tco.color.getRGB());
            GL11.glDisable(3042);
        ...
        if (tco.drainEntity != null && tco.drainCollision != null) {
            ...
            if (drainEntity instanceof EntityPlayer && !((EntityPlayer)drainEntity).isUsingItem()) {
                ...
                UtilsFX.drawFloatyLine(drainCollision.blockX + 0.5, drainCollision.blockY + 0.5, drainCollision.blockZ + 0.5, d3, d4 + d6, d5, f, tco.color.getRGB(), "textures/misc/wispy.png", -0.02f, Math.min(iiud, 10) / 10.0f);
                GL11.glPopMatrix();
            }
        }
        GL11.glPushMatrix();
