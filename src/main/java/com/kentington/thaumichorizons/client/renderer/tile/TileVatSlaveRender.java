        ...
        if (te.getBlockMetadata() == 0 && te.getWorldObj().getBlockMetadata(te.xCoord, te.yCoord + 1, te.zCoord) == 7) {
            ...
            if (tco.getEntityContained() != null && !(tco.getEntityContained() instanceof EntityPlayer)) {
                if (tco.mode == 1) {
                    ...
                    final float f2 = p_147936_1_.prevRotationYaw + (p_147936_1_.rotationYaw - p_147936_1_.prevRotationYaw) * f;
                ...
                GL11.glTranslatef((float)(-x) - 0.5f, (float)(-y) - 1.5f + 0.1f * (float)Math.cos(Math.toRadians(Minecraft.getMinecraft().thePlayer.ticksExisted)), (float)z + 0.5f);
                ...
                if (this.stack == null || tco.sample.getItem() != this.stack.getEntityItem().getItem()) {
                    this.stack = new EntityItem(tco.getWorldObj(), tco.xCoord + 0.5, tco.yCoord - 1.0, tco.zCoord + 0.5, tco.sample);
                }
                GL11.glTranslatef(0.0f, 0.1f * (float)Math.cos(Math.toRadians(Minecraft.getMinecraft().thePlayer.ticksExisted)), 0.0f);
