...
public class TilePortalTHRender extends TileEntitySpecialRenderer
{
    ...
    private void renderPortal(final TilePortalTH te, final double x, final double y, final double z, final float f) {
        ...
        if (Minecraft.getMinecraft().renderViewEntity instanceof EntityPlayer) {
            ...
            tessellator.addVertexWithUV(px + v1.xCoord * scale, py + v1.yCoord * scaley, pz + v1.zCoord * scale, (double)f2, (double)f5);
            tessellator.addVertexWithUV(px + v2.xCoord * scale, py + v2.yCoord * scaley, pz + v2.zCoord * scale, (double)f3, (double)f5);
            tessellator.addVertexWithUV(px + v3.xCoord * scale, py + v3.yCoord * scaley, pz + v3.zCoord * scale, (double)f3, (double)f4);
            tessellator.addVertexWithUV(px + v4.xCoord * scale, py + v4.yCoord * scaley, pz + v4.zCoord * scale, (double)f2, (double)f4);
            tessellator.draw();
        }
        GL11.glDisable(3042);
