...
public class TileSoulforgeRender extends TileEntitySpecialRenderer
{
    ...
    public void renderTileEntityAt(final TileEntity tile, final double x, final double y, final double z, final float f) {
        ...
        if (((TileSoulforge)tile).forging > 0) {
            ...
            GL11.glPushMatrix();
            UtilsFX.renderFacingQuad(tile.xCoord + 0.025, tile.yCoord + 0.75, tile.zCoord + 0.025, 0.0f, 0.2f, 0.9f, frames, i, f, 16777215);
            GL11.glPopMatrix();
            GL11.glPushMatrix();
            UtilsFX.renderFacingQuad(tile.xCoord + 0.975, tile.yCoord + 0.75, tile.zCoord + 0.025, 0.0f, 0.2f, 0.9f, frames, i, f, 16777215);
            GL11.glPopMatrix();
            GL11.glPushMatrix();
            UtilsFX.renderFacingQuad(tile.xCoord + 0.975, tile.yCoord + 0.75, tile.zCoord + 0.975, 0.0f, 0.2f, 0.9f, frames, i, f, 16777215);
            GL11.glPopMatrix();
            GL11.glPushMatrix();
            UtilsFX.renderFacingQuad(tile.xCoord + 0.025, tile.yCoord + 0.75, tile.zCoord + 0.975, 0.0f, 0.2f, 0.9f, frames, i, f, 16777215);
            GL11.glPopMatrix();
            GL11.glDisable(3042);
        }
        if (((TileSoulforge)tile).souls > 0) {
            ...
            for (int which = 0; which < ((TileSoulforge)tile).souls; ++which) {
                ...
                GL11.glEnable(3042);
                GL11.glBlendFunc(770, 771);
                UtilsFX.renderFacingQuad(tile.xCoord + 0.5 + dist * Math.sin(2.0 * radian + offset * which), tile.yCoord + 0.85, tile.zCoord + 0.5 + dist * Math.cos(2.0 * radian + offset * which), 0.0f, 0.1f, 0.9f, frames, (int)(nt / 40000000L % frames), f, 16777215);
                GL11.glDisable(3042);
                GL11.glPopMatrix();
            }
            GL11.glEnable(2884);
