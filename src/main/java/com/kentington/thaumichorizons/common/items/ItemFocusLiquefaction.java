...
public class ItemFocusLiquefaction extends ItemFocusBasic
{
    ...
    @Override
    public ItemStack onFocusRightClick(final ItemStack itemstack, final World world, final EntityPlayer p, final MovingObjectPosition mop) {
    ...
    public void onUsingFocusTick(final ItemStack stack, final EntityPlayer p, final int count) {
        ...
        if (!wand.consumeAllVis(stack, p, this.getVisCost(stack), false, true)) {
            ...
            if (p.worldObj.isRemote) {
                ItemFocusLiquefaction.beam.put(pp, Thaumcraft.proxy.beamCont(p.worldObj, p, tx, ty, tz, 0, 16729156, false, (impact > 0) ? ((float)size) : 0.0f, ItemFocusLiquefaction.beam.get(pp), 5));
            }
            if ((ent == null || (!(ent instanceof EntityLiving) && (!(ent instanceof EntityItem) || FurnaceRecipes.smelting().getSmeltingResult(((EntityItem)ent).getEntityItem()) == null))) && mop != null && mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                final boolean flammable = bi.isFlammable((IBlockAccess)p.worldObj, mop.blockX, mop.blockY, mop.blockZ, ForgeDirection.UNKNOWN);
                if (meltable > 0 || flammable || FurnaceRecipes.smelting().getSmeltingResult(new ItemStack(bi, 1, md)) != null) {
                    final int pot = wand.getFocusPotency(stack);
                    final float speed = 0.15f + pot * 0.05f;
                    float hardness = 2.0f;
                    if (meltable > 0 || bi.isFlammable((IBlockAccess)p.worldObj, mop.blockX, mop.blockY, mop.blockZ, ForgeDirection.UP)) {
                        hardness = 0.25f;
                    }
                    if (meltable == 6) {
                        hardness = 20.0f;
                    }
                    if (ItemFocusLiquefaction.lastX.get(pp) == mop.blockX && ItemFocusLiquefaction.lastY.get(pp) == mop.blockY && ItemFocusLiquefaction.lastZ.get(pp) == mop.blockZ) {
            ...
            else if (ent != null && ent instanceof EntityLiving && wand.consumeAllVis(stack, p, ItemFocusLiquefaction.costCritter, true, true)) {
                ...
                if (wand.consumeAllVis(stack, p, this.getVisCost(stack), true, true)) {
                    ...
                    final ItemStack stacky = FurnaceRecipes.smelting().getSmeltingResult(((EntityItem)ent).getEntityItem());
    ...
    public FocusUpgradeType[] getPossibleUpgradesByRank(final ItemStack focusstack, final int rank) {
        switch (rank) {
            case 1: {
                return new FocusUpgradeType[] { FocusUpgradeType.frugal, FocusUpgradeType.potency, ItemFocusLiquefaction.nuggets };
            }
            case 2: {
                return new FocusUpgradeType[] { FocusUpgradeType.frugal, FocusUpgradeType.potency, ItemFocusLiquefaction.nuggets };
            }
            ...
            case 5: {
                return new FocusUpgradeType[] { FocusUpgradeType.frugal, FocusUpgradeType.potency, ItemFocusLiquefaction.purity };
            }
            default: {
                return null;
            }
    ...
    public static Entity getPointedEntity(final World world, final EntityLivingBase entityplayer, final double range) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (world.rayTraceBlocks(Vec3.createVectorHelper(entityplayer.posX, entityplayer.posY + entityplayer.getEyeHeight(), entityplayer.posZ), Vec3.createVectorHelper(entity.posX, entity.posY + entity.getEyeHeight(), entity.posZ), false) == null) {
    ...
    void processBlock(final int x, final int y, final int z, final ItemWandCasting wand, final ItemStack stack, final EntityPlayer p, final String pp) {
        ...
        if (result != null && wand.consumeAllVis(stack, p, this.getVisCost(stack), true, true)) {
            if (this.getUpgradeLevel(wand.getFocusItem(stack), ItemFocusLiquefaction.purity) > 0 && Utils.findSpecialMiningResult(new ItemStack(bi, 1, md), 9999.0f, p.worldObj.rand) != null) {
                final ItemStack pure = Utils.findSpecialMiningResult(new ItemStack(bi, 1, md), 9999.0f, p.worldObj.rand);
                if (FurnaceRecipes.smelting().getSmeltingResult(pure) != null) {
                    result = FurnaceRecipes.smelting().getSmeltingResult(pure);
                }
            }
            if (this.getUpgradeLevel(wand.getFocusItem(stack), ItemFocusLiquefaction.nuggets) > 0 && ThaumcraftApi.getSmeltingBonus(new ItemStack(bi, 1, md)) != null) {
                final ItemStack bonus = ThaumcraftApi.getSmeltingBonus(new ItemStack(bi, 1, md)).copy();
                for (int i = 0; i < this.getUpgradeLevel(wand.getFocusItem(stack), ItemFocusLiquefaction.nuggets); ++i) {
                ...
                if (bonus.stackSize > 0) {
                    final EntityItem bonusEntity = new EntityItem(p.worldObj, (double)(x + 0.5f), (double)(y + 0.5f), (double)(z + 0.5f), bonus);
            ...
            if (result.getItem() instanceof ItemBlock) {
                ...
                final EntityItem theItem = new EntityItem(p.worldObj, (double)(x + 0.5f), (double)(y + 0.5f), (double)(z + 0.5f));
    ...
    static {
        ItemFocusLiquefaction.nuggets = new FocusUpgradeType(FocusUpgradeType.types.length, new ResourceLocation("thaumichorizons", "textures/foci/nuggets.png"), "focus.upgrade.nuggets.name", "focus.upgrade.nuggets.text", new AspectList().add(Aspect.METAL, 8));
        ItemFocusLiquefaction.purity = new FocusUpgradeType(FocusUpgradeType.types.length, new ResourceLocation("thaumichorizons", "textures/foci/purity.png"), "focus.upgrade.purity.name", "focus.upgrade.purity.text", new AspectList().add(Aspect.ORDER, 8));
