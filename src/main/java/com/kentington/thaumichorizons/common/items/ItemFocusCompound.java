...
public class ItemFocusCompound extends ItemFocusBasic
{
    ...
    public Aspect chooseRandomFilteredFromSource(final AspectList filter, final boolean preserve, final AspectList nodeAspects, final World world) {
    ...
    public void onUsingFocusTick(final ItemStack wandstack, final EntityPlayer player, final int count) {
        ...
        final MovingObjectPosition movingobjectposition = BlockUtils.getTargetBlock(player.worldObj, (Entity)player, true);
        int i = 0;
        int j = 0;
        int k = 0;
        ...
        if (count % 5 == 0) {
            ...
            final boolean preserve = !player.isSneaking() && ResearchManager.isResearchComplete(player.getCommandSenderName(), "NODEPRESERVE") && !wand.getRod(wandstack).getTag().equals("wood") && !wand.getCap(wandstack).getTag().equals("iron");
            ...
            if (success) {
                ...
                Thaumcraft.proxy.beamPower(player.worldObj, i + 0.5, j + 0.5, k + 0.5, player.posX, player.posY + player.eyeHeight, player.posZ, col.getRed() / 255.0f, col.getGreen() / 255.0f, col.getBlue() / 255.0f, true, (Object)node);
            }
            if (mfu) {
    ...
    static {
        ItemFocusCompound.fission = new FocusUpgradeType(FocusUpgradeType.types.length, new ResourceLocation("thaumichorizons", "textures/foci/fission.png"), "focus.upgrade.fission.name", "focus.upgrade.fission.text", new AspectList().add(Aspect.EXCHANGE, 8));
        cost = new AspectList().add(Aspect.FIRE, 0).add(Aspect.WATER, 0).add(Aspect.AIR, 0).add(Aspect.EARTH, 0).add(Aspect.ORDER, 0).add(Aspect.ENTROPY, 0);
    }
}
