    ...
    public ItemStack onItemRightClick(final ItemStack p_77659_1_, final World world, final EntityPlayer p) {
        ...
        if (ent != null && ent instanceof EntityLiving && this.isValidInfusionTarget((EntityLiving)ent)) {
            ...
            for (final CreatureInfusionRecipe recipe : ThaumicHorizons.critterRecipes) {
                ...
                if (recipe.getID(null) == p_77659_1_.getItemDamage() && recipe.getRecipeOutput() instanceof NBTTagCompound && !((EntityInfusionProperties)critter.getExtendedProperties("CreatureInfusion")).hasInfusion(recipe.getID(null))) {
                    ...
                    if (tagMods.getDouble("generic.movementSpeed") > 0.0) {
                        map.put((Object)"generic.movementSpeed", (Object)new AttributeModifier("generic.movementSpeed", tagMods.getDouble("generic.movementSpeed") / 10.0, 1));
                    }
                    if (tagMods.getDouble("generic.maxHealth") > 0.0) {
                        map.put((Object)"generic.maxHealth", (Object)new AttributeModifier("generic.maxHealth", tagMods.getDouble("generic.maxHealth"), 1));
                    }
                    if (tagMods.getDouble("generic.attackDamage") > 0.0) {
                        map.put((Object)"generic.attackDamage", (Object)new AttributeModifier("generic.attackDamage", tagMods.getDouble("generic.attackDamage"), 1));
                    }
                    if (map.size() > 0) {
                    ...
                    if (!blockLoyalty) {
                        ...
                        Thaumcraft.proxy.burst(world, critter.posX, critter.posY + critter.getEyeHeight(), critter.posZ, 1.0f);
                    }
                    return p_77659_1_;
                }
            }
