...
public class ItemFocusDisintegration extends ItemFocusBasic
{
    ...
    public void onUsingFocusTick(ItemStack stack, final EntityPlayer p, final int count) {
        ...
        if (!wand.consumeAllVis(stack, p, this.getVisCost(stack), false, true)) {
            ...
            if (p.worldObj.isRemote) {
                ItemFocusDisintegration.beam.put(pp, Thaumcraft.proxy.beamCont(p.worldObj, p, tx, ty, tz, 0, 7930047, false, (impact > 0) ? ((float)size) : 0.0f, ItemFocusDisintegration.beam.get(pp), 5));
            }
            if ((ent == null || (!(ent instanceof EntityLiving) && (!(ent instanceof EntityItem) || ((EntityItem)ent).getEntityItem().getItem() == ConfigItems.itemCrystalEssence || this.getAspects(((EntityItem)ent).getEntityItem().getItem(), ((EntityItem)ent).getEntityItem().getItemDamage()) == null || this.getAspects(((EntityItem)ent).getEntityItem().getItem(), ((EntityItem)ent).getEntityItem().getItemDamage()).size() == 0))) && mop != null && mop.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (Item.getItemFromBlock(bi) != null && this.getAspects(Item.getItemFromBlock(bi), md) != null) {
                    ...
                    if (ItemFocusDisintegration.lastX.get(pp) == mop.blockX && ItemFocusDisintegration.lastY.get(pp) == mop.blockY && ItemFocusDisintegration.lastZ.get(pp) == mop.blockZ) {
            ...
            else if (ent != null && ent instanceof EntityLiving && wand.consumeAllVis(stack, p, ItemFocusDisintegration.costCritter, true, true)) {
                ...
                ((EntityLiving)ent).attackEntityFrom(DamageSourceThaumcraft.dissolve, 0.5f + 0.25f * wand.getFocusPotency(stack) + 0.5f * this.getUpgradeLevel(wand.getFocusItem(stack), ItemFocusDisintegration.enervation));
            }
                ...
                if (wand.consumeAllVis(stack, p, this.getVisCost(stack), true, true)) {
                    ...
                    final AspectList aspects = this.getAspects(((EntityItem)ent).getEntityItem().getItem(), ((EntityItem)ent).getEntityItem().getItemDamage());
    ...
    public FocusUpgradeType[] getPossibleUpgradesByRank(final ItemStack focusstack, final int rank) {
        switch (rank) {
            ...
            case 5: {
                return new FocusUpgradeType[] { FocusUpgradeType.frugal, FocusUpgradeType.potency, ItemFocusDisintegration.enervation };
            }
            default: {
                return null;
            }
    ...
    public static Entity getPointedEntity(final World world, final EntityLivingBase entityplayer, final double range) {
        ...
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (!(entity instanceof EntityItem) || ((EntityItem)entity).getEntityItem().getItem() != ConfigItems.itemCrystalEssence) {
    ...
    void processBlock(final int x, final int y, final int z, final ItemWandCasting wand, ItemStack stack, final EntityPlayer p, final String pp) {
        ...
        if (Item.getItemFromBlock(bi) != null && this.getAspects(Item.getItemFromBlock(bi), md) != null && this.getAspects(Item.getItemFromBlock(bi), md).size() > 0 && wand.consumeAllVis(stack, p, this.getVisCost(stack), true, true)) {
            final AspectList aspects = this.getAspects(Item.getItemFromBlock(p.worldObj.getBlock(x, y, z)), p.worldObj.getBlockMetadata(x, y, z));
            if (aspects != null) {
                for (final Aspect asp : aspects.getAspects()) {
                    stack = new ItemStack(ConfigItems.itemCrystalEssence, aspects.getAmount(asp));
                    ((ItemCrystalEssence)stack.getItem()).setAspects(stack, new AspectList().add(asp, 1));
                    p.worldObj.spawnEntityInWorld((Entity)new EntityItemInvulnerable(p.worldObj, x + 0.5, y + 0.5, z + 0.5, stack));
                }
    ...
    static {
        ItemFocusDisintegration.enervation = new FocusUpgradeType(FocusUpgradeType.types.length, new ResourceLocation("thaumichorizons", "textures/foci/enervation.png"), "focus.upgrade.enervation.name", "focus.upgrade.enervation.text", new AspectList().add(Aspect.DEATH, 8));
