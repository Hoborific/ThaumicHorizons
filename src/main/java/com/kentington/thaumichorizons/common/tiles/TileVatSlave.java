    ...
    public TileVat getBoss(final int mdOverride) {
        if (!this.bossFound) {
            ...
            if (md == 0) {
                ...
                for (int x = -1; x < 2; ++x) {
                    for (int z = -1; z < 2; ++z) {
                        if (this.worldObj.getBlock(this.xCoord + x, this.yCoord, this.zCoord + z) == ThaumicHorizons.blockVatInterior && this.worldObj.getBlockMetadata(this.xCoord + x, this.yCoord, this.zCoord + z) == 0 && this.worldObj.getTileEntity(this.xCoord + x, this.yCoord, this.zCoord + z) instanceof TileVatSlave) {
                            final TileVat boss = ((TileVatSlave)this.worldObj.getTileEntity(this.xCoord + x, this.yCoord, this.zCoord + z)).getBoss(-1);
                            if (boss != null) {
                ...
                if (this.worldObj.getBlock(this.xCoord, this.yCoord + 1, this.zCoord) == ThaumicHorizons.blockVat && this.worldObj.getTileEntity(this.xCoord, this.yCoord + 1, this.zCoord) instanceof TileVatSlave) {
                    final TileVat boss2 = ((TileVatSlave)this.worldObj.getTileEntity(this.xCoord, this.yCoord + 1, this.zCoord)).getBoss(-1);
                    if (boss2 != null) {
