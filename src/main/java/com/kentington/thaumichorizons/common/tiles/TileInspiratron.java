...
public class TileInspiratron extends TileThaumcraft implements ISoulReceiver, ISidedInventory
{
    ...
    public void updateEntity() {
        ...
        if (this.worldObj.isRemote) {
            ...
            this.rotb = this.rota;
            if (entity == null) {
                entity = (Entity)this.worldObj.getClosestPlayer((double)(this.xCoord + 0.5f), (double)(this.yCoord + 0.5f), (double)(this.zCoord + 0.5f), 6.0);
            }
            if (entity != null) {
    ...
    public void addSoulBits(final int bits) {
        for (int i = 0; i < bits; ++i) {
            if (Math.random() >= 0.97) {
                this.worldObj.playSoundEffect(this.xCoord + 0.5, this.yCoord + 0.5, this.zCoord + 0.5, "thaumcraft:write", 0.2f, this.worldObj.rand.nextFloat());
                ++this.progress;
            }
        }
