    ...
    public void updateEntity() {
        ...
        if ((above instanceof TileJarBrain && ((TileJarBrain)above).xp < ((TileJarBrain)above).xpMax) || (above instanceof ISoulReceiver && ((ISoulReceiver)above).canAcceptSouls())) {
            ...
            if (!this.worldObj.isRemote && this.ticksLeft > 0) {
                final int visBoost = VisNetHandler.drainVis(this.worldObj, this.xCoord, this.yCoord, this.zCoord, Aspect.AIR, 10);
                ...
                if (this.ticksLeft <= 0) {
                    ...
                    if (below != null && below instanceof ISidedInventory) {
                        ...
                        final int[] slots = accessibleSlotsFromSide = ((ISidedInventory)below).getAccessibleSlotsFromSide(1);
                        ...
                        for (int slots2 = ((IInventory)below).getSizeInventory(), k = 0; k < slots2; ++k) {
                            if (((IInventory)below).getStackInSlot(k) == null || ((IInventory)below).getStackInSlot(k).getItem() == sand.getItem()) {
                                InventoryUtils.placeItemStackIntoInventory(sand, (IInventory)below, 1, true);
                                break;
                            }
                        }
                        ...
                        final EntityItem fallenSand = new EntityItem(this.worldObj, this.xCoord + 0.5, (double)this.yCoord, this.zCoord + 0.5, sand);
                ...
                if (this.sieveMotion >= 360) {
                    ...
                    this.worldObj.playSound(this.xCoord + 0.5, this.yCoord + 0.5, this.zCoord + 0.5, "dig.sand", 1.0f, 0.0f, false);
