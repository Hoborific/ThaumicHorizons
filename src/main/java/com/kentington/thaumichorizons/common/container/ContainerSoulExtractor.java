...
public class ContainerSoulExtractor extends Container
{
    ...
    public ContainerSoulExtractor(final InventoryPlayer p_i1812_1_, final TileSoulExtractor p_i1812_2_) {
        ...
        this.addSlotToContainer((Slot)new SlotRestricted((IInventory)p_i1812_2_, 0, 64, 30, new ItemStack(Blocks.soul_sand)));
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
    ...
    public ItemStack transferStackInSlot(final EntityPlayer p_82846_1_, final int p_82846_2_) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (p_82846_2_ == 0) {
                ...
                if (itemstack.getItem() != Item.getItemFromBlock(Blocks.soul_sand) || !this.mergeItemStack(itemstack2, 0, 1, false)) {
                    return null;
                }
                slot.onSlotChange(itemstack2, itemstack);
                if (itemstack2.stackSize == 0) {
