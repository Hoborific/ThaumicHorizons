...
public class ContainerCase extends Container
{
    ...
    public ContainerCase(final InventoryPlayer iinventory, final World par2World, final int par3, final int par4, final int par5) {
        ...
        for (int a = 0; a < 18; ++a) {
            this.addSlotToContainer((Slot)new SlotLimitedByClass((Class)ILens.class, this.input, a, 37 + a % 6 * 18, 51 + a / 6 * 18));
        }
    ...
    protected void bindPlayerInventory(final InventoryPlayer inventoryPlayer) {
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 9; ++j) {
                this.addSlotToContainer(new Slot((IInventory)inventoryPlayer, j + i * 9 + 9, 8 + j * 18, 151 + i * 18));
            }
        }
        for (int i = 0; i < 9; ++i) {
