...
public class ExplosionAlchemite extends Explosion
{
    ...
    public ExplosionAlchemite(final World p_i1948_1_, final Entity p_i1948_2_, final double p_i1948_3_, final double p_i1948_5_, final double p_i1948_7_, final float p_i1948_9_) {
    ...
    public void doExplosionA() {
                    ...
                    if (i == 0 || i == this.field_77289_h - 1 || j == 0 || j == this.field_77289_h - 1 || k == 0 || k == this.field_77289_h - 1) {
                        ...
                        for (float f3 = 0.3f; f2 > 0.0f; f2 -= f3 * 0.75f) {
                            ...
                            if (this.worldObj.getTileEntity(j2, k2, l1) != null && this.worldObj.getTileEntity(j2, k2, l1) instanceof TileNode) {
                                ...
                                final float f4 = (this.exploder != null) ? this.exploder.func_145772_a((Explosion)this, this.worldObj, j2, k2, l1, block) : block.getExplosionResistance(this.exploder, this.worldObj, j2, k2, l1, this.explosionX, this.explosionY, this.explosionZ);
                                f2 -= (f4 + 0.3f) * f3;
                            }
        ...
        final List list = this.worldObj.getEntitiesWithinAABBExcludingEntity(this.exploder, AxisAlignedBB.getBoundingBox((double)i, (double)k, (double)m, (double)j, (double)i2, (double)j3));
        ...
        for (int i3 = 0; i3 < list.size(); ++i3) {
            ...
            if (d8 <= 1.0) {
                ...
                if (d9 != 0.0) {
                    ...
                    entity.attackEntityFrom(DamageSourceThaumcraft.dissolve, (float)(int)((d11 * d11 + d11) / 2.0 * 16.0 * this.explosionSize + 1.0));
    ...
    public void doExplosionB(final boolean p_77279_1_) {
        ...
        if (this.isSmoking) {
            for (final ChunkPosition chunkposition : this.affectedBlockPositions) {
                ...
                if (this.worldObj.getTileEntity(i, j, k) != null && this.worldObj.getTileEntity(i, j, k) instanceof TileNode) {
                    ...
                    if (d < 0.25) {
                        ...
                        if (aspects != null) {
                            for (final Aspect asp : aspects.getAspects()) {
                                final ItemStack stack = new ItemStack(ConfigItems.itemCrystalEssence, aspects.getAmount(asp));
                    ...
                    final AspectList aspects = this.getAspects(Item.getItemFromBlock(block), this.worldObj.getBlockMetadata(i, j, k));
                    if (aspects != null && aspects.size() > 0) {
                        for (final Aspect asp2 : aspects.getAspects()) {
                            final ItemStack stack = new ItemStack(ConfigItems.itemCrystalEssence, aspects.getAmount(asp2));
        ...
        if (this.isFlaming) {
            for (final ChunkPosition chunkposition : this.affectedBlockPositions) {
                ...
                if (block.getMaterial() == Material.air && block2.func_149730_j() && this.explosionRNG.nextInt(3) == 0) {
    ...
    public EntityLivingBase getExplosivePlacedBy() {
        return (this.exploder == null) ? null : (EntityLivingBase) ((this.exploder instanceof EntityTNTPrimed) ? ((EntityTNTPrimed) this.exploder).getTntPlacedBy() : ((this.exploder instanceof EntityLivingBase) ? this.exploder : null));
    }
}
