...
public class PacketFXInfusionDone implements IMessage, IMessageHandler<PacketFXInfusionDone, IMessage>
{
    ...
    public IMessage onMessage(final PacketFXInfusionDone message, final MessageContext ctx) {
        Thaumcraft.proxy.blockSparkle((World)Minecraft.getMinecraft().theWorld, message.x, message.y, message.z, -9999, 20);
        Thaumcraft.proxy.blockSparkle((World)Minecraft.getMinecraft().theWorld, message.x, message.y - 1, message.z, -9999, 20);
        return null;
    }
