...
public class EntityAIHurtByTargetTH extends EntityAITargetTH
{
    ...
    public void startExecuting() {
        ...
        if (this.entityCallsForHelp) {
            ...
            final List<EntityLiving> list = this.taskOwner.worldObj.getEntitiesWithinAABB((Class)this.taskOwner.getClass(), AxisAlignedBB.getBoundingBox(this.taskOwner.posX, this.taskOwner.posY, this.taskOwner.posZ, this.taskOwner.posX + 1.0, this.taskOwner.posY + 1.0, this.taskOwner.posZ + 1.0).expand(d0, 10.0, d0));
            for (final EntityLiving EntityLiving : list) {
                if (this.taskOwner != EntityLiving && EntityLiving.getAttackTarget() == null && !EntityLiving.isOnSameTeam(this.taskOwner.getAITarget())) {
