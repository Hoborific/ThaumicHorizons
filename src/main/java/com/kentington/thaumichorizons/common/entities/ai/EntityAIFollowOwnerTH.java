...
public class EntityAIFollowOwnerTH extends EntityAIBase
{
    ...
    public EntityAIFollowOwnerTH(final EntityLiving p_i1625_1_, final double p_i1625_2_, final float p_i1625_4_, final float p_i1625_5_) {
    ...
    public boolean shouldExecute() {
        final EntityInfusionProperties prop = (EntityInfusionProperties)this.thePet.getExtendedProperties("CreatureInfusion");
        final EntityLivingBase entitylivingbase = (EntityLivingBase)this.thePet.worldObj.getPlayerEntityByName(prop.getOwner());
    ...
    public boolean continueExecuting() {
        final EntityInfusionProperties prop = (EntityInfusionProperties)this.thePet.getExtendedProperties("CreatureInfusion");
        return !this.petPathfinder.noPath() && this.thePet.getDistanceSqToEntity((Entity)this.theOwner) > this.maxDist * this.maxDist && !prop.isSitting();
    }
            ...
            if (!this.petPathfinder.tryMoveToEntityLiving((Entity)this.theOwner, this.field_75336_f) && !this.thePet.getLeashed() && this.thePet.getDistanceSqToEntity((Entity)this.theOwner) >= 144.0) {
                ...
                for (int l = 0; l <= 4; ++l) {
                    for (int i2 = 0; i2 <= 4; ++i2) {
                        if ((l < 1 || i2 < 1 || l > 3 || i2 > 3) && World.doesBlockHaveSolidTopSurface((IBlockAccess)this.theWorld, i + l, k - 1, j + i2) && !this.theWorld.getBlock(i + l, k, j + i2).isNormalCube() && !this.theWorld.getBlock(i + l, k + 1, j + i2).isNormalCube()) {
